...
shaka.media.PresentationTimeline = class {
  ...
  getSafeSeekRangeStart(
    ...
    // console.log(`MS: desiredStart=${desiredStart}`);
  ...
  getSeekRangeEnd() {
    ...
    // console.log(`MS: getSegmentAvailabilityEnd = ${this.getSegmentAvailabilityEnd()}, delay=${delay}`);
  ...
  getLiveEdge_() {
    ...
    // Date.now() is in milliseconds, from which we compute "now" in seconds.
    const now = (Date.now() + this.clockOffset_) / 1000.0;
    const liveEdge = now - this.maxSegmentDuration_ - this.presentationStartTime_;
    // console.log(`MS: now=${now}, maxSeg=${this.maxSegmentDuration_}, startTime=${this.presentationStartTime_}, liveEdge=${liveEdge}`);
    // console.log(`MS: now=${new Date(now*1000).toLocaleString('en-US')}, ` + 
    //               `liveEdge=${new Date((this.presentationStartTime_+liveEdge)*1000).toLocaleString('en-US')}`);
